-- FUNCTION: olapts.panegftest(date)

-- DROP FUNCTION olapts.panegftest(date);

CREATE OR REPLACE FUNCTION olapts.panegftest(
	ref_date date)
    RETURNS boolean
    LANGUAGE 'plpgsql'
    COST 100
...
begin
	...
	pl_targettablename:='panegftest';
	
	truncate TABLE olapts.panegftest; 
...
and a.ApprovedDate <= cast(ref_date as date) + time '23:59:59'--'2023-09-29 23:59:59' 
...
ALTER FUNCTION olapts.panegftest(date)
    OWNER TO uniadmintrn;

--select * from olapts.panegftest('2023-09-29')	
